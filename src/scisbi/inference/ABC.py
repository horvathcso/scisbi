...
class ABCRejectionSampling(BaseInferenceAlgorithm):
    ...
    def infer(
        ...
        **kwargs: Any,
        ...
        if verbose:
            print(f"Starting ABC rejection sampling...")
        ...
        while len(accepted_samples) < num_simulations:
            ...
            simulated_data = self.simulator.simulate(theta_candidate, num_simulations=len(observed_summary))
            ...
            if distance <= tolerance:
                ...
                if verbose and len(accepted_samples) % max(1, num_simulations // 10) == 0:
                    acceptance_rate = len(accepted_samples) / attempt_count if attempt_count > 0 else 0
                    print(f"Progress: {len(accepted_samples)}/{num_simulations} "
        ...
        if verbose:
            ...
            print(f"ABC rejection sampling completed!")
...
class ABCPosterior:
    ...
    def sample(self, num_samples: int = 1) -> Union[Any, List[Any]]:
        ...
        if num_samples == 1:
            ...
            return np.random.choice(self.samples, size=num_samples, replace=True).tolist()
    ...
    def plot(self, parameter_index: Optional[int] = None, bins: int = 30) -> None:
        ...
        if parameter_index is None:
            if isinstance(self.samples[0], (list, tuple)):
                raise ValueError("Samples are multi-dimensional. Specify parameter_index.")
        ...
        title = f"Posterior Distribution" + (f" (Parameter {parameter_index})" if parameter_index is not None else "")
        plt.title(title)
        plt.xlabel(f"Parameter {parameter_index}" if parameter_index is not None else "Samples")
        plt.ylabel("Density")
        plt.grid(True)
        plt.show()
    ...
    def __repr__(self) -> str:
        return (f"ABCPosterior(num_samples={len(self.samples)}, "
